...
export function CategoryTable() {
  ...
  const columns: Array<ColumnDef<Event>> = useMemo(
    () => [
      ...
      ...(events_query.data?.events[0]
        ? Object.keys(events_query.data.events[0].fields as object).map(
            (field) => ({
  ...
  const table = useReactTable({
    ...
    pageCount: Math.ceil(
      (events_query.data?.events_count || 0) / context.pagination.pageSize
    ),
    ...
    <Fragment>
      ...
      <div className="flex items-center justify-end space-x-2 py-4">
        <Button
          ...
          disabled={!table.getCanPreviousPage() || events_query.isFetching}>
          Previous
        </Button>
        <Button
          variant="outline"
          ...
          disabled={!table.getCanNextPage() || events_query.isFetching}>
          Next
        </Button>
      </div>
    </Fragment>
